...
class ValueIteration(AbstractSolver):
    ...
    def train_episode(self):
        ...
        # Initialize an array for storing the updated values temporarily
        updated_values = np.zeros(self.env.nS)
        ...
        for each_state in range(self.env.nS):
            ...
            # Iterate over possible actions in the state
            for action in range(self.env.nA):
                # Initialize a variable to store the expected value of the action
                action_value = 0

                # Perform a one-step lookahead to calculate the action value
                for probability, next_state, reward, done in self.env.P[each_state][action]:
                    # Use the Bellman Equation to calculate the value
                    action_value += probability * (reward + self.options.gamma * self.V[next_state])

                # Append the calculated value to the list of action values
                action_values.append(action_value)

                # Update the value of the state to the best action value
                updated_values[each_state] = max(action_values)

        # Update the state-value function with the newly computed values
        self.V = updated_values
            ###################################################
    ...
    def create_greedy_policy(self):
        ...
        def policy_fn(state):
            ...
            ################################
            #   YOUR IMPLEMENTATION HERE   #
            # Initialize a variable to store the best action and its value
            best_action = None
