...
class A2C(AbstractSolver):
    ...
    def train(self, states, actions, rewards, next_state, done):
        ...
        G = 0
        if not done:
            G = self.actor_critic.value(torch.tensor(next_state, dtype=torch.float32)).detach()
        ...
        critic_loss = torch.square(returns - values)  # Mean squared error loss
    
        loss = policy_loss.mean() + critic_loss.mean()
        self.optimizer.zero_grad()  # Reset gradients to zero before backpropagation
        loss.backward()
        self.optimizer.step()
