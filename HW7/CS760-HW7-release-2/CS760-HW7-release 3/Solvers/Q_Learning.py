...
class QLearning(AbstractSolver):
    ...
    def train_episode(self):
        ...
        for _ in range(self.options.steps):
            ...
            # Get the best possible future reward from the next state
            best_future_reward = np.max(self.Q[next_state])

            # Calculate the updated Q value using the Q-learning update rule
            self.Q[state][action] = self.Q[state][action] + \
                                    self.options.alpha * (reward + self.options.gamma * best_future_reward - self.Q[state][action])

            # Transition to the next state
            state = next_state

            # Break out of the loop if the end of the episode is reached
            if done:
                break
        ################################
